...
function mostrarResumen() {
  ...
  carrito.forEach((producto) => {
    ...
    item.textContent = `${producto.nombre} x${producto.cantidad} - L ${subtotal.toFixed(2)}`;
  ...
  if (carrito.length === 0) {
    lista.innerHTML = "<li>No hay productos en tu carrito.</li>";
...
function validarFormulario() {
  ...
  // Validación simple de teléfono (solo números, espacios y guiones, mínimo 8 dígitos)
  const telefono = document.getElementById("telefono").value.trim();
  const soloNumeros = telefono.replace(/[^0-9]/g, "");
  if (telefono !== "" && soloNumeros.length < 8) {
    document.getElementById("error-telefono").textContent = "Ingresa un teléfono válido (mínimo 8 dígitos).";
    esValido = false;
  }
...
// Guarda los datos del pedido completo (cliente + productos + total)
...
function manejarEnvio(evento) {
  ...
  // TODO (Ana): antes de redirigir, aquí debe completarse el pago con
  // PayPal Sandbox y el envío del pedido a Formspree. Por ahora,
  // el flujo redirige directo a confirmacion.html para poder probar
  // que el resto del checkout funciona de principio a fin.
  window.location.href = "confirmacion.html";
}

document.addEventListener("DOMContentLoaded", () => {
  mostrarResumen();
  document.getElementById("form-checkout").addEventListener("submit", manejarEnvio);
});